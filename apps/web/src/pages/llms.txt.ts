...
import { getRootNodes, getAllPaths } from '@/lib/corpus';
...
export const GET: APIRoute = async () => {
  ...
  const docCount = (await getAllPaths(db)).length;

  const baskets = roots
    .map((node) => {
      const desc = PITAKA_BLURBS[node.slug] ?? node.blurb;
      const suffix = desc ? `: ${desc}` : '';
      return `- [${node.pali}](${site.url}/${node.slug})${suffix}`;
    })
    .join('\n');
...
Paragraphs carry CST page and PTS volume.page references. Each paragraph is deep-linkable as #para-N (e.g. ${site.url}/sutta/kn/dhammapadapali/yamakavaggo#para-1).
